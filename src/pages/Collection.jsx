...
// Bulk Import function parseBulkText(raw) {
const lines = raw.split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('//') && !l.startsWith('#'))
if (lines.length === 0) return { rows: [], format: 'empty' }
// CSV detection: first line contains a comma and a word like "name" const firstLower = lines[0].toLowerCase()
...
// Line-by-line formats const rows = []
let format = 'plain'
...
// Batch-fetch Scryfall data (up to 75 per request) const uniqueNames = [...new Set(parsed.map(r => r.name))]
...
// Enrich parsed rows with Scryfall data const enriched = parsed.map(row => {
...
// Bulk insert / update let added = 0, updated = 0
const errors = []
try {
...
>
{localForSale ? '✓ Sell Binder' : ' Sell Binder'}
</button>
</div>
<button
...
// Binder persistence // forSale / forTrade don't live in Supabase, so we track them in a separate
...
// Filter state const [filterColors, setFilterColors] = useState([])
...
// One-time migration: backfill scryfallId for existing collection cards // Runs in the background after collection loads. Uses the Scryfall /cards/collection
...
// helpers function updateCard(id, patch) {
...
// Trade binder const tradeCards = collection.filter(c => c.forTrade)
...
// Filtering (memoized) const filtered = useMemo(() => {
...
['sell', ` Sell${forSaleCount > 0 ? ` (${forSaleCount})` : ''}`],
['trade', ` Trade${forTradeCount > 0 ? ` (${forTradeCount})` : ''}`],
].map(([id, label]) => (
<button
key={id}
onClick={() => {
...
// Insurance / Print Report Modal function InsuranceModal({ collection, onClose }) {
const totalValue = collection.reduce((s, c) => s + (parseFloat(c.price) || 0) * (c.qty || 1), 0)
...
<div style={{ flex: 1 }}>
<div style={{ fontWeight: 800, fontSize: '1rem' }}> Insurance / Print Report</div>
...
// Sell List row function SellCard({ card, onUpdatePrice, onUpdateQty, onRemoveFromSell, ckBuyPrice, abuBuyPrice, scgBuyPrice, scgHotlist }) {
...
// Trade Binder row function TradeCard({ card, onRemove }) {
return (
<div style={{
background: 'var(--bg-card)', border: '1px solid var(--border)',
borderRadius: 'var(--radius)', padding: '12px 14px',
