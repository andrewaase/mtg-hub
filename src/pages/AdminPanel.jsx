...
// Stat card function StatCard({ icon, label, value, sub, color = '#f59e0b' }) {
return (
<div style={{
...
// Mini bar chart (signups over 30 days) function SignupChart({ signupsByDay }) {
...
// Set membership (admin action) async function setUserMembership(userId, tier, months, token) {
...
// Tier badge function TierBadge({ tier }) {
...
// User table function UserTable({ users, onTierChange, onBan }) {
...
// Listings tab const CONDITIONS = ['NM', 'LP', 'MP', 'HP', 'DMG']
...
// Edit listing modal function EditListingModal({ listing, onClose, onSaved }) {
...
// Bulk CSV import modal function BulkImportModal({ onClose, onSaved }) {
...
// Inventory stats const stats = useMemo(() => {
...
// Filter + sort const filtered = useMemo(() => {
let out = listings
...
// Orders tab const STATUS_COLORS = {
...
// Settings tab function SettingsTab() {
...
<div style={{ fontWeight: 800, fontSize: '1rem', color: '#f1f5f9', marginBottom: 4 }}> Store Settings</div>
...
// Main component // Reports tab function ReportsTab() {
...
// Meta Tracker admin tab const META_FORMATS_ADMIN = ['Standard','Pioneer','Modern','Legacy','Pauper','Commander','Brawl','Vintage','Explorer','Premodern']
...
// Art card picker state const [artPickerOpen, setArtPickerOpen] = useState(false)
...
<div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#f1f5f9', letterSpacing: '-0.5px' }}> Control Center</div>
</div>
<div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
{tab === 'overview' && (
