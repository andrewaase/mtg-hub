...
// Sync listing price when card or foil mode changes useEffect(() => {
...
// Core scan async function scanFrame() {
...
// Add to collection async function handleAdd(options = {}) {
...
// Undo a recent add async function handleUndo(entry, index) {
...
// Manual name correction (re-lookup with a corrected name) async function handleManualCorrect() {
...
// Add to store inventory (admin only) async function handleAddToStore() {
if (!foundCard || adding) return
...
// Rapid Mode auto-add // When enabled, auto-adds the identified card after RAPID_DELAY_MS so the
...
// Derived const DFC_LAYOUTS = ['transform', 'modal_dfc', 'reversible_card', 'double_faced_token']
