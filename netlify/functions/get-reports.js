// netlify/functions/get-reports.js
// Admin-only: returns all submitted reports, newest first.
...
exports.handler = async (event) => {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/reports?select=*&order=created_at.desc&limit=200`,
  ...
  return {
    statusCode: 200,
    headers: { 'Content-Type': 'application/json', ...corsHeaders(event) },
    body: JSON.stringify({ reports: Array.isArray(reports) ? reports : [] }),
  }
}
